...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
    if (info.menuItemId === 'translate-selection' && info.selectionText) {
        try {
            ...
            if (tab?.id && tab.id >= 0 && tab.url && !tab.url.startsWith('chrome://') && !tab.url.startsWith('chrome-extension://')) {
                try {
                    ...
                    try {
                        ...
                        setTimeout(async () => {
                            if (!tab.id || tab.id < 0)
                                return; // Validate tab.id before using
                            try {
                                await chrome.tabs.sendMessage(tab.id, {
                                    action: 'showTranslation',
                                    translation: data.translation
                        ...
                        }, 500); // Increased timeout to allow more time for script loading
                    }
                    catch (injectError) {
